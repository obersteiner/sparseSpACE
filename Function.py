...
class GenzProductPeak(Function):
    ...
    def getAnalyticSolutionIntegral(self,start,end):
        result = 1
        for d in range(self.dim):
            result *= np.arctan(self.coeffs[d]*(self.midPoint[d] - start[d])) * self.coeffs[d] - np.arctan(self.coeffs[d]*(self.midPoint[d] - end[d])) * self.coeffs[d]
        return result * self.factor
    ...
    def getAnalyticSolutionIntegral(self,start,end):
        ...
        for d in range(self.dim):
            ...
            if(start[d] < self.midPoint[d]):
                oneDIntegral += (np.exp(self.coeffs[d]*(start[d] - self.midPoint[d])) - 1) / self.coeffs[d]
            if(end[d] > self.midPoint[d]):
                oneDIntegral += (np.exp(self.coeffs[d]*(end[d] - self.midPoint[d])) - 1) / self.coeffs[d]
            result *= oneDIntegral
        return result
...
class Function2Jakeman(Function):
    ...
    def getAnalyticSolutionIntegral(self,start,end):
        ...
        for d in range(dim):
            result = result * (sqPiHalve * scipy.special.erf(np.sqrt(self.coefficients[d]) * (end[d] - self.midpoints[d])) -
                               sqPiHalve * scipy.special.erf(np.sqrt(self.coefficients[d]) * (start[d] - self.midpoints[d])) )  / np.sqrt(self.coefficients[d])
        return result

#This function is the first test function of the paper from Gerstner and Griebel:http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.33.3141&rep=rep1&type=pdf
class FunctionGriebel(Function):
    ...
    def getAnalyticSolutionIntegral(self,start,end):
        ...
        result = 1.0
        for d in range(dim):
            result = result * (end[d]**(1+1.0/dim) / (1+1.0/dim) - start[d]**(1+1.0/dim) / (1+1.0/dim))
