...
class SpatiallyAdaptiveSingleDimensions(SpatiallyAdaptivBase):
    def __init__(self, a, b, grid):
        ...
        self.errorEstimator = ErrorCalculatorVolumeGuidedPunishedDepth()
    ...
    def evaluate_area(self, f, area, levelvec):
        ...
        if(self.grid.is_global() == True):
            gridPointCoordsAsStripes = self.get_point_coord_for_each_dim(levelvec)
            start = self.a
            end = self.b
            dim = self.dim
            widthPerDimension = self.get_num_points_all_dim(levelvec)
            # initializes grid-specific refinement information
            self.init_grid_specific_info(levelvec, widthPerDimension)

            integral, partial_integral, evaluations, metaRefineContainer = self.grid.integrator.integrate_hierarchical(f, gridPointCoordsAsStripes, start, end, widthPerDimension, self.refinement, levelvec, self.lmin, self.lmax)
