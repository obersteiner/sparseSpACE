...
class SpatiallyAdaptivBase(StandardCombi):
    ...
    def evaluate_operation(self) -> Tuple[float, float]:
        ...
        self.errorVector=self.refinement.errorVector 
        if self.print_output:
    ...
    def continue_adaptive_refinement(self, tol: float=10 ** -3, max_time: float=None, max_evaluations: int=None, min_evaluations: int=1) -> Tuple[RefinementContainer, Sequence[ComponentGridInfo], Sequence[int], Sequence[float], Sequence[float], Sequence[int], Sequence[float]]:
        ...
        while True:
            ...
            self.errorVectorStorage[self.get_total_num_points(distinct_function_evals=True)]=(self.errorVector) 
        ...
        return self.refinement, self.scheme, self.lmax, combi_result, number_of_evaluations, self.error_array, self.num_point_array, self.surplus_error_array, self.interpolation_error_arrayL2, self.interpolation_error_arrayMax,self.errorVectorStorage
